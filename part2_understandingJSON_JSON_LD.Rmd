...
Now that we know the background of JSON-LD, we can learn the most important parts, aka keys, used in a JSON-LD file. Let`s dive into it. 
:::

### Type

As a first step we can now add a `type` to our JSON file. A node type specifies the type of thing that is being described, like a person, place, event, or web page. Each of the JSON datasets is describing a person so we can search [Schema.org](https://schema.org/) for `Person` and add the link of the type to our JSON file. This step will just be explained with the first example.
...
What we have seen now is the `context` and the `type` that was added to the JSON. Still not clear? Not a problem my friend. Have a look at the following statement from [JSON-LD.org](https://json-ld.org/spec/latest/json-ld/#the-context) that may help you to understand the concept of jSON-LD:

:::{.message}
When two people communicate with one another, the conversation takes place in a shared environment, typically called "the context of the conversation". This shared context allows the individuals to use shortcut terms, like the first name of a mutual friend, to communicate more quickly but without losing accuracy. A context in JSON-LD works in the same way. It allows two applications to use shortcut terms to communicate with one another more efficiently, but without losing accuracy
:::

In other words, when we are talking about anything we are basically considering the context that something happened or will happen, this can also be used with linked data in the JSON-LD format. So in the examples above we used the "@context" key to all of the three shown JSON files.
...
```{r quiz2, echo=FALSE}
quiz(caption = "Quiz - How JSON-LD work?",
  question("What is the main difference between JSON-LD to a regular JSON file?"),
    answer("JSON-LD is used for linked data", correct = TRUE, message = "JSON-LD facilitates the communication with linked data. JSON has no built-in support for hyperlinks, which are a fundamental building block on the Web. Remember that the 'LD' stands for linked data."),
    answer("There is no difference between both of them, JSON-LD is just another name that is used to call the JSON format."),
    answer("JSON-LD has a completely diferent structure when comparing to a JSON."),
  question("What is the most important 'key' used in a JSON-LD file?"),
    answer("@json-ld"),
    answer("@main"),
    answer("@background"),
    answer("@info"),
    answer("@context", correct = TRUE, message = "The '@context' tells the application how to interpret the rest of the data in the document. It allows two applications to use shortcut terms to communicate with one another more efficiently, but without losing accuracy.")
  )
```
